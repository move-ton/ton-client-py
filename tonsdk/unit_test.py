...
class TestTonContracts(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.client.setup(settings=lib.TON_CLIENT_DEFAULT_SETUP)

    def test_tvm_get(self):
        result = self.client.contracts.tvm_get(
            function_name="participant_list", code_b64=self.code,
            data_b64=self.data)

        self.assertEqual(result["success"], True)
        self.assertGreater(len(result["result"]["output"]), 0)
    ...
    def test_load(self):
        result = self.client.contracts.load(address=self.address)

        self.assertEqual(result["success"], True)
        self.assertEqual(result["result"]["id"], "2ac9e24350f1940158e017602126b48407cbb59c9c840eb4fdf58692dc864ff6")

    def test_find_shard(self):
        result = self.client.contracts.find_shard(
            address=self.address, shards=self.shards)

        self.assertEqual(result["success"], True)

    def test_convert_address(self):
        result = self.client.contracts.convert_address(
            address=self.address, convert_to="AccountId")
        self.assertEqual(result["result"]["address"], "2ac9e24350f1940158e017602126b48407cbb59c9c840eb4fdf58692dc864ff6")

        result = self.client.contracts.convert_address(
            address=self.address, convert_to="Hex")
        self.assertEqual(result["result"]["address"], self.address)

        result = self.client.contracts.convert_address(
            address=self.address, convert_to="Base64",
            b64_params={"url": False, "test": True, "bounce": True})
        self.assertEqual(result["result"]["address"], "kf8qyeJDUPGUAVjgF2AhJrSEB8u1nJyEDrT99YaS3IZP9tac")
