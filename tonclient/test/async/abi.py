...
class TestTonCryptoAsync(aiounittest.AsyncTestCase):
    ...
    async def test_encode_message(self):
        ...
        call_set = CallSet(
            ...
            header={'pubkey': keypair.public, 'time': time, 'expire': expire})
        signer = Signer(data=keypair.public)
        ...
        signature = await client.crypto.sign(
            unsigned=unsigned['data_to_sign'], keys=keypair)
        self.assertEqual(
        ...
        signed = await client.abi.attach_signature(
            abi=self.events_abi, public_key=keypair.public,
