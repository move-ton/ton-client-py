...
class Terminal(object):
    def __init__(self, messages: List[str]):
        ...
        self.methods = {'print': self.print, 'inputInt': self.input_int}
...
class DebotBrowser(object):
    ...
    class AppDebot(AppDebotBrowser):
        ...
        def perform_show_action(self, params: ParamsOfAppDebotBrowser.ShowAction):
            logging.info(
                f'[ACTION]\t{len(self.browser.actions)}: ' f'{params.action.__str__()}'
            )
        ...
        def perform_get_signing_box(self) -> int:
            ...
            result = self.client.crypto.get_signing_box(params=self.browser.keypair)
        ...
        def perform_approve(self, params: ParamsOfAppDebotBrowser.Approve) -> bool:
    ...
    def init_debot(self):
        app_debot_browser = DebotBrowser.AppDebot(client=self.client, browser=self)
    ...
    def handle_message_queue(self):
        while len(self.messages):
            ...
            if wc == str(DEBOT_WC):
                ...
                call_set = (
                    CallSet(function_name=hex(func_id), input=kwargs)
                    if func_id > 0
                    else None
                )
                ...
                params_send = ParamsOfSend(
                    debot_handle=self.debot.debot_handle, message=internal.message
                )
                ...
                if not debot_fetched:
                    _browser = DebotBrowser(
                        client=self.client, address=dest_address, keypair=self.keypair
                    )
                    _browser.init_debot()

                debot_fetched = self.debots[dest_address].debot_handle

                params_send = ParamsOfSend(debot_handle=debot_fetched, message=msg_opt)
                self.client.debot.send(params=params_send)
...
class DebotBrowserAsync(object):
    ...
    class AppDebot(AppDebotBrowser):
        ...
        async def perform_show_action(self, params: ParamsOfAppDebotBrowser.ShowAction):
            logging.info(
                f'[ACTION]\t{len(self.browser.actions)}: ' f'{params.action.__str__()}'
            )
            self.browser.actions.append(params.action)

        async def perform_input(self, params: ParamsOfAppDebotBrowser.Input) -> str:
            logging.info(f"[INPUT]\t{self.browser.step['inputs'][0]}")
            return self.browser.step['inputs'][0]
    ...
    async def init_debot(self):
        app_debot_browser = DebotBrowserAsync.AppDebot(client=self.client, browser=self)
...
class TestTonDebotAsyncCore(unittest.TestCase):
    ...
    def test_print(self):
        ...
        steps = [
            ...
            {'choice': 0, 'inputs': [], 'outputs': ['test simple print']},
            {
                'choice': 1,
                'inputs': [],
                'outputs': [f'integer=1,addr={target_address},string=test_string_1'],
            },
    ...
    def test_send_message(self):
        ...
        browser = DebotBrowser(
            client=async_core_client,
            address=debot_address,
            steps=steps,
    ...
    def test_debot_msg_interface(self):
        ...
        terminal_outputs = ['counter=10', 'Increment succeeded', 'counter=15']
    ...
    def test_debot_invoke_msgs(self):
        ...
        browser = DebotBrowser(
            client=async_core_client, address=debot_a, terminal_outputs=terminal_outputs
        )
        browser.init_debot()
        browser.execute()
    ...
    def test_debot_sdk_get_accounts_by_hash(self):
        ...
        params = ParamsOfAggregateCollection(
            collection='accounts',
            filter={'code_hash': {'eq': contract_hash}},
    ...
    def test_debot_json_interface(self):
        ...
        browser = DebotBrowser(client=async_core_client, address=debot_address)
        browser.init_debot()
        browser.execute()
    ...
    def test_invoke_async(self):
        ...
        async def __main():
            browser = DebotBrowserAsync(
                client=client, address=debot_address, steps=steps, keypair=self.keypair
            )
    ...
    def __init_debot(self, keypair: KeyPair) -> Tuple[str, str]:
        ...
        debot_abi = Abi.from_path(path=os.path.join(SAMPLES_DIR, 'Debot.abi.json'))
        ...
        encode_params = ParamsOfEncodeMessage(
            abi=self.target_abi, signer=signer, deploy_set=deploy_set, call_set=call_set
        )
        ...
        if not target_address:
            ...
            target = async_core_client.processing.process_message(params=process_params)
        ...
        encode_params = ParamsOfEncodeMessage(
            abi=debot_abi, signer=signer, deploy_set=deploy_set, call_set=call_set
        )
        message = async_core_client.abi.encode_message(params=encode_params)
        ...
        if not debot_address:
            ...
            debot = async_core_client.processing.process_message(params=process_params)
    ...
    def __init_debot2(self, keypair: KeyPair) -> str:
        signer = Signer.Keys(keys=keypair)
        debot_abi = Abi.from_path(path=os.path.join(SAMPLES_DIR, 'Debot2.abi.json'))
        with open(os.path.join(SAMPLES_DIR, 'Debot2.tvc'), 'rb') as fp:
            debot_tvc = base64.b64encode(fp.read()).decode()

        call_set = CallSet(
            function_name='constructor',
            input={'pub': f'0x{keypair.public}', 'sec': f'0x{keypair.secret}'},
        )
        deploy_set = DeploySet(tvc=debot_tvc)
        encode_params = ParamsOfEncodeMessage(
            abi=debot_abi, signer=signer, deploy_set=deploy_set, call_set=call_set
    ...
    def __init_debot4(self, keypair: KeyPair) -> Tuple[str, str]:
        signer = Signer.Keys(keys=keypair)
        debot_abi = Abi.from_path(path=os.path.join(SAMPLES_DIR, 'Debot4.abi.json'))
        ...
        if not debot_address:
            ...
            call_set = CallSet(
                ...
                input={'image': self.target_tvc, 'pubkey': f'0x{keypair.public}'},
            )
            async_core_client.processing.process_message(
                params=ParamsOfProcessMessage(
                    message_encode_params=ParamsOfEncodeMessage(
    ...
    def __init_debot5(self, count: int) -> Tuple[List[str], str]:
        debot_abi = Abi.from_path(path=os.path.join(SAMPLES_DIR, 'Debot5.abi.json'))
        ...
        call_set = CallSet(
            function_name='constructor', input={'codeHash': f'0x{contract_hash}'}
        )
        deploy_set = DeploySet(tvc=debot_tvc)
        ...
        for i in range(count):
            ...
            # Calculate address
            message = async_core_client.abi.encode_message(params=deploy_params)
            ...
            # Set ABI
            call_set = CallSet(
                function_name='setABI', input={'dabi': debot_abi.value.encode().hex()}
            )
            async_core_client.processing.process_message(
                params=ParamsOfProcessMessage(
                    message_encode_params=ParamsOfEncodeMessage(
                        abi=debot_abi,
                        signer=deploy_params.signer,
                        address=debot_address,
                        call_set=call_set,
                    ),
                    send_events=False,
    ...
    def __init_debot_pair(self, keypair: KeyPair) -> Tuple[str, str]:
        ...
        encode_params = ParamsOfEncodeMessage(
            abi=debot_b_abi, signer=signer, deploy_set=deploy_set, call_set=call_set
        )
        message = async_core_client.abi.encode_message(params=encode_params)
        ...
        debot_b = async_core_client.processing.process_message(params=process_params)
        b_address = debot_b.transaction['account_addr']

        # Set ABI
        call_set = CallSet(
            function_name='setAbi', input={'debotAbi': debot_b_abi.value.encode().hex()}
        )
        async_core_client.processing.process_message(
            params=ParamsOfProcessMessage(
                message_encode_params=ParamsOfEncodeMessage(
                    abi=debot_b_abi, signer=signer, address=b_address, call_set=call_set
                ),
                send_events=False,
            )
        )
        ...
        # Deploy debot A
        call_set = CallSet(function_name='constructor', input={'targetAddr': b_address})
        deploy_set = DeploySet(tvc=debot_a_tvc)
        encode_params = ParamsOfEncodeMessage(
            abi=debot_a_abi, signer=signer, deploy_set=deploy_set, call_set=call_set
        )
        message = async_core_client.abi.encode_message(params=encode_params)
        ...
        debot_a = async_core_client.processing.process_message(params=process_params)
        a_address = debot_a.transaction['account_addr']

        # Set ABI
        call_set = CallSet(
            function_name='setAbi', input={'debotAbi': debot_a_abi.value.encode().hex()}
        )
        async_core_client.processing.process_message(
            params=ParamsOfProcessMessage(
                message_encode_params=ParamsOfEncodeMessage(
                    abi=debot_a_abi, signer=signer, address=a_address, call_set=call_set
                ),
                send_events=False,
            )
        )
    ...
    def __init_simple_debot(self, name: str, keypair: KeyPair) -> str:
        ...
        signer = Signer.Keys(keys=keypair)
        debot_abi = Abi.from_path(path=os.path.join(SAMPLES_DIR, f'{name}.abi.json'))
    ...
    def __check_address(address: str) -> Union[str, None]:
        ...
        q_params = ParamsOfQueryCollection(
            collection='accounts', result='id', filter={'id': {'eq': address}}
        )
