...
from tonclient.test.crypto import client
from tonclient.types import KeyPair, MnemonicDictionary

client.is_async = True


class TestTonCryptoAsync(aiounittest.AsyncTestCase):
    def setUp(self) -> None:
    ...
    async def test_sha256(self):
        ...
        result = await client.crypto.sha256(data=data)
        self.assertEqual(
    ...
    async def test_sha512(self):
        data = base64.b64encode('TON is our future'.encode())
        result = await client.crypto.sha512(data=data)
        self.assertEqual(
    ...
    async def test_hdkey_xprv_from_mnemonic(self):
        xprv = await client.crypto.hdkey_xprv_from_mnemonic(
            phrase=self.mnemonic)
        self.assertEqual(self.master_xprv, xprv)

        with self.assertRaises(TonException):
            await client.crypto.hdkey_xprv_from_mnemonic(phrase=0)

    async def test_hdkey_secret_from_xprv(self):
        secret = await client.crypto.hdkey_secret_from_xprv(
        ...
        with self.assertRaises(TonException):
            await client.crypto.hdkey_secret_from_xprv(xprv='')

    async def test_hdkey_public_from_xprv(self):
        public = await client.crypto.hdkey_public_from_xprv(xprv=self.master_xprv)
        self.assertEqual(
            '02a8eb63085f73c33fa31b4d1134259406347284f8dab6fc68f4bf8c96f6c39b75',
            public)

    async def test_hdkey_derive_from_xprv(self):
        child_xprv = await client.crypto.hdkey_derive_from_xprv(
        ...
        with self.assertRaises(TonException):
            await client.crypto.hdkey_derive_from_xprv(
                xprv=self.master_xprv, child_index=-1, hardened=False)

    async def test_hdkey_derive_from_xprv_path(self):
        xprv = await client.crypto.hdkey_derive_from_xprv_path(
        ...
        with self.assertRaises(TonException):
            await client.crypto.hdkey_derive_from_xprv_path(
                xprv=self.master_xprv, path='m/')

    async def test_convert_public_key_to_ton_safe_format(self):
        safe = await client.crypto.convert_public_key_to_ton_safe_format(
        ...
        with self.assertRaises(TonException):
            await client.crypto.convert_public_key_to_ton_safe_format(
                public_key=None)

    async def test_generate_random_sign_keys(self):
    ...
    async def test_sign_and_verify(self):
        ...
        # Sign message
        signed = await client.crypto.sign(unsigned=unsigned, keys=keypair)
        self.assertEqual(
        ...
        # Verify signature
        verified = await client.crypto.verify_signature(
        ...
        with self.assertRaises(TonException):
            await client.crypto.sign(
                unsigned=unsigned, keys=KeyPair(public='1', secret='2'))

        with self.assertRaises(TonException):
            await client.crypto.verify_signature(
                signed='simple', public=keypair.public)

    async def test_modular_power(self):
        result = await client.crypto.modular_power(
            base='0123456789ABCDEF', exponent='0123', modulus='01234567')
        self.assertEqual('63bfdf', result)

        with self.assertRaises(TonException):
            await client.crypto.modular_power(
                base='1', exponent='0123', modulus='0.2')

    async def test_factorize(self):
        factors = await client.crypto.factorize(composite='17ED48941A08F981')
        self.assertIsInstance(factors, list)
        self.assertEqual('494C553B', factors[0])
        self.assertEqual('53911073', factors[1])

        with self.assertRaises(TonException):
            await client.crypto.factorize(composite='a3')

    async def test_ton_crc16(self):
        data = base64.b64encode(bytes.fromhex('0123456789abcdef'))
        crc = await client.crypto.ton_crc16(data=data)
        self.assertEqual(43349, crc)

        with self.assertRaises(TonException):
            await client.crypto.ton_crc16(data='0==')

    async def test_generate_random_bytes(self):
        bytes_b64 = await client.crypto.generate_random_bytes(length=32)
        self.assertEqual(44, len(bytes_b64))
        bts = base64.b64decode(bytes_b64.encode())
        self.assertEqual(32, len(bts))

        with self.assertRaises(TonException):
            await client.crypto.generate_random_bytes(length='1')

    async def test_mnemonic_words(self):
        words = await client.crypto.mnemonic_words()
        self.assertEqual(2048, len(words.split(' ')))

        with self.assertRaises(TonException):
            await client.crypto.mnemonic_words(dictionary=100)

    async def test_mnemonic_from_random(self):
        for count in [12, 15, 18, 21, 24]:
            mnemonic = await client.crypto.mnemonic_from_random(
                word_count=count)
            self.assertEqual(count, len(mnemonic.split(' ')))

        with self.assertRaises(TonException):
            await client.crypto.mnemonic_from_random(word_count=0)

    async def test_mnemonic_from_entropy(self):
        mnemonic = await client.crypto.mnemonic_from_entropy(
        ...
        with self.assertRaises(TonException):
            await client.crypto.mnemonic_from_entropy(entropy='01')

    async def test_mnemonic_verify(self):
        for count in [12, 15, 18, 21, 24]:
            mnemonic = await client.crypto.mnemonic_from_random(
                word_count=count)
            valid = await client.crypto.mnemonic_verify(
                phrase=mnemonic, word_count=count)
            self.assertEqual(True, valid)

        valid = await client.crypto.mnemonic_verify(phrase='one')
        self.assertEqual(False, valid)

    async def test_mnemonic_derive_sign_keys(self):
        ...
        keypair = await client.crypto.mnemonic_derive_sign_keys(
            phrase=phrase, dictionary=MnemonicDictionary.TON, word_count=24)
        public_safe = await client.crypto.convert_public_key_to_ton_safe_format(
        ...
        keypair = await client.crypto.mnemonic_derive_sign_keys(phrase=phrase)
        public_safe = await client.crypto.convert_public_key_to_ton_safe_format(
            public_key=keypair.public)
        self.assertEqual(
        ...
        keypair = await client.crypto.mnemonic_derive_sign_keys(phrase=mnemonic)
        public_safe = await client.crypto.convert_public_key_to_ton_safe_format(
            public_key=keypair.public)
        self.assertEqual(
            'PuZdw_KyXIzo8IksTrERN3_WoAoYTyK7OvM-yaLk711sUIB3', public_safe)

    async def test_nacl_sign_keypair_from_secret_key(self):
        keypair = await client.crypto.nacl_sign_keypair_from_secret_key(
        ...
        with self.assertRaises(TonException):
            await client.crypto.nacl_sign_keypair_from_secret_key(secret='0a')

    async def test_nacl_sign(self):
        # Nacl sign data
        unsigned = base64.b64encode('Test Message'.encode())
        ...
        signed = await client.crypto.nacl_sign(
        ...
        # Nacl sign open
        opened = await client.crypto.nacl_sign_open(
        ...
        # Nacl sign detached
        signature = await client.crypto.nacl_sign_detached(
            unsigned=unsigned, secret=secret)
        self.assertEqual(
            'fb0cfe40eea5d6c960652e6ceb904da8a72ee2fcf6e05089cf835203179ff65bb48c57ecf31dcfcd26510bea67e64f3e6898b7c58300dc14338254268cade103',
            signature)
        ...
        with self.assertRaises(TonException):
            await client.crypto.nacl_sign(unsigned='0==', secret=secret)

        with self.assertRaises(TonException):
            await client.crypto.nacl_sign_open(signed=signed, public='0x00')

        with self.assertRaises(TonException):
            await client.crypto.nacl_sign_detached(unsigned=100, secret=secret)

    async def test_nacl_box_keypair(self):
        keypair = await client.crypto.nacl_box_keypair()
        self.assertEqual(64, len(keypair.public))
        self.assertEqual(64, len(keypair.secret))
        self.assertNotEqual(keypair.public, keypair.secret)

    async def test_nacl_box_keypair_from_secret_key(self):
        keypair = await client.crypto.nacl_box_keypair_from_secret_key(
        ...
        with self.assertRaises(TonException):
            await client.crypto.nacl_box_keypair_from_secret_key(secret='0x00')

    async def test_nacl_box_and_open(self):
    ...
    async def test_nacl_secret_box_and_open(self):
        decrypted = base64.b64encode('Test Message \' \" {} $=,?'.encode())
    ...
    async def test_scrypt(self):
        ...
        key = await client.crypto.scrypt(
